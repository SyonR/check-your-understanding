...
`index.html` contains all lesson rendering, navigation, responsive styling, embedded reveal/checkpoint activities, quiz behavior, scoring, review links, and the compact project-guide subpage from `curriculum.guide`. Embedded activities are ungraded and preserve their state while the learner navigates. The Project Guide leads directly to the quiz; there is no separate quiz-introduction page. Question order and each question's choice order are randomized on initial load and rebuilt for every full retake while remaining stable during an attempt. Its maintained shell uses the IBM Design Language UI palette for every session: neutral grays dominate, Blue 60 is the primary action color, and green/red are reserved for status feedback. The same palette is applied to Mermaid diagrams. The shell uses a full-viewport content area and a collapsible numbered sidebar. The sidebar has no title/header, its scroll indicator stays hidden, and the lesson area has no top header or progress bar. Mermaid 11.17.2 is loaded from the adjacent local asset and renders `mermaid` fenced blocks offline with `securityLevel: strict`.
